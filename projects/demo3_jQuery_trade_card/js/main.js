...
$(document).ready(    function () {
    $("#readyMessage").fadeIn(500);

    setTimeout(function () {
        $("#readyMessage").fadeOut(500);
    }, 1000);
});

var bShowHide = 1;// 1 currently hidden. 0 currently shown.
$(function () {
    $("#showSkills").click(function () {
        if (bShowHide == 0)
        {
            $("#aboutMe").slideUp(1000);
            $("#showSkills").val("Show Skills");
        } else {
            $("#aboutMe").slideDown(1000);
            $("#showSkills").val("Hide Skills");
        }
        bShowHide = 1 - bShowHide;
    });
});